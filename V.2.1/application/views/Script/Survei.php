...
<script>
	...
	function total_survei(id, n, d) {
		// $('#form_survei')[0].reset();
		$('#variable_survei_total tbody').empty();
		// $('#metode').val('update')
		// $('#id_hasil').val(id)
		url = '<?= base_url('Survei/getIndikator/') ?>' + id;
		data = $('#filter-form').serialize();
		$.ajax({
			url: url,
			type: "POST",
			...
			success: function(data) {
				...
				var nemu = "<tr><td>" + data.nemulator_variable + "</td>" +
					"<td contenteditable='true' placeholder='Masukan Nilai Nemulator...' id='nemu'>" + n + "</td>" +
					"<td>" + data.nemulator_satuan + "</td>" +
					"</tr>" +
					"<tr><td>" + data.demulator_variable + "</td>" +
					"<td contenteditable='true' placeholder='Masukan Nilai Demulator...' id='demu'>" + d + "</td>" +
					"<td>" + data.demulator_satuan + "</td>" +
					"</tr>";
				$('#variable_survei_total tbody').append(nemu);
				console.log(nemu);
