...
<script>
    ...
    $('#unitkat').change(function () { 
        // var unit = $('#unitkat').val();
        valunit = $(this).val()
        console.log(valunit);
        
        $('#indikatorunit').select2({
            allowClear: true,
            ajax: {
				url: '<?=base_url('Unitindikator/getIndikator/')?>'+valunit,
				type: "post",
				dataType: 'json',
				delay: 250,
	...
	function add() {
		remove_validation();
		$("#indikatorunit").val('').trigger("change");
		$("#unitkat").val('').trigger("change");
		$('#form_input')[0].reset();
	...
	$("#tables tbody").on("click", "#edit", function () {
		...
		$.ajax({
			...
			dataType: "JSON",
			success: function (data) {
				$("#unitkat").val(data.unitkat).trigger("change");
				$("#indikatorunit").val(data.indikatorunit).trigger("change");
				$("#idunitindikator").val(data.idunitindikator);
				// $('#detail').attr('onclick',indikatorunit);
	...
	$("#tables tbody").on("click", "#delBtn", function () {
		remove_validation();
		// $('#form_del')[0].reset();
		$('.modal-title').text('Hapus Data Unit');
		$('#modal-del').modal({
			backdrop: "static",
			show: true
		});
	...
	$('#detail').click(function () { 
		...
		$.ajax({
			type: "method",
			url: "<?=base_url('Unitindikator/Detailindikator/')?>"+indi,
			dataType: "JSON",
			success: function (data) {
